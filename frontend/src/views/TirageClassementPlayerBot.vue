...
export default {
  ...
  data() {
    return {
      ...
      bots: [] // Propriété pour stocker les bots
    };
  },
  computed: {
  ...
  created() {
    ...
    if (savedGrid) {
      ...
      this.fetchBots(); // Récupérer les bots
    } else {
  ...
  methods: {
    ...
    async fetchPlayers() {
      ...
      if (this.selectedPlayers.length === 0) {
        ...
        this.players = response.data; // Stocker les joueurs récupérés
      } else {
    ...
    /**
     * Récupère la liste des bots depuis l'API.
     * @returns {Promise<void>}
     */
    async fetchBots() {
      // Récupérer les bots via l'API
      const response = await axios.get('http://localhost:5001/api/users/bots');
      this.bots = response.data; // Stocker les bots récupérés
      this.players = [...this.players, ...this.bots]; // Ajouter les bots aux joueurs
      this.calculateWinners(); // Recalculer les gagnants avec les bots inclus
    },
    ...
    calculateWinnings(index) {
      ...
      // Trouver tous les gagnants ayant le même nombre de correspondances
      ...
      if (tiedWinners.length > 1) {
        ...
        const totalPercentage = tiedIndices.reduce((sum, idx) => sum + this.rankPercentages[idx], 0); // Somme des pourcentages

        // Calculer la part de chaque gagnant égal
        const share = totalPercentage / tiedWinners.length; // Part pour chaque gagnant

        return (this.totalPrize * share).toFixed(2); // Retourner les gains formatés
      } else {
        // Si pas d'égalité, calculer normalement
